...
class ValidacaoEdicaoUsuarioMiddleware implements IMiddlwareValidar {
  ...
  public async Validar(AReq: Request): Promise<void> {
    const cpfValido: boolean = false || this._utils.ValidarCPF(AReq['body']['usuario']['cpf']);
    ...
    // await check(`usuario.cpf`).exists({ checkNull: true }).withMessage(this._validacaoServices.NaoPodeSerNulo(`usuario.cpf`)).run(AReq);
    // await check(`usuario.cpf`).custom(() => cpfValido).withMessage(this._validacaoServices.DeveConterCPFValido(`usuario.cpf`)).run(AReq);
    // await check(`usuario.cpf`).isLength({ min: 11, max: 11 }).withMessage(this._validacaoServices.DeveConterXCaracteres(`usuario.cpf`, 11)).run(AReq);
    // await check(`usuario.cpf`).isNumeric({ no_symbols: true }).withMessage(this._validacaoServices.DeveConterColecaoStringNumerica(`usuario.cpf`)).run(AReq);

    // await check(`usuario.data_nascimento`).optional({ nullable: true }).notEmpty().withMessage(this._validacaoServices.NaoPodeSerVazio(`usuario.data_nascimento`)).run(AReq);
    // await check(`usuario.data_nascimento`).optional({ nullable: true }).isISO8601().withMessage(this._validacaoServices.DeveConterDATAValida(`usuario.data_nascimento`)).run(AReq);
    ...
    // await check('usuario.cep').isLength({ min: 8, max: 8 }).withMessage(this._validacaoServices.DeveConterXDigitos('usuario.municipio', 8)).run(AReq);

  }
}

export default new ValidacaoEdicaoUsuarioMiddleware();